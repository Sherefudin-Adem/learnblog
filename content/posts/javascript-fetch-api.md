...
```js
const request = new Request('https://some url please/api/users', {
    headers: headers
});
...
const request = new Request('https://some url please/users');
The Request object also accepts a URL object:

const url = new URL('https://some url please/api/users');
...
const fetchUsers = async () => {
    try {
        const response = await fetch('https://some url please/api/users');
        if (!response.ok) {
